...
class Nao (Robot):
    ...
    def show_welcome_message(self):
        ...
        # self.display.fillRectangle(0, 0, 20, self.display.getHeight())
    
        self.display.setColor(0xFFFFFF)  # White text
        self.display.setFont("Arial", 12, True)
    
        # adjust line breaks based on your display width
        lines ="Step into a masterpiece!\nPose, pick a style,and \nbecome part of art history!"
        
   
        self.display.drawText(lines, 1, 1)
    ...
    # ... existing code ...
        while robot.step(self.timeStep) != -1:
            ...
            if robot.getKeyboard().getKey() == ord('C'):
                # Get image from Webots camera
                self.content_image = printCameraImage(camera)
                
                # Apply style transfer (van Gogh by default)
                styled_img = apply_style(content_img)
                
                # Show on display (from previous steps)
                display.imagePaste(cv2.cvtColor(styled_img, cv2.COLOR_BGR2RGBA), 0, 0)
    ...
    def findAndEnableDevices(self):
        ...
        # display 
        self.display = self.getDevice("ChestBoard/display")
        ...
        self.display = self.getDevice("chest_display")  # Must match your display name in Webots
        # self.display.enable(self.timeStep)
